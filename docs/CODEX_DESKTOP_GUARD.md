...
When the guard reports `codex-cli-or-approved-local`:

```text
Persist task manifest and continuation state
→ route repository work to Codex CLI
...
3. Use CLI or approved local execution until the old session is archived and recovery-tested.
...
3. In-app browser navigation passes ten controlled read-only canaries on Windows.
4. Large-thread focus switching and resume tests remain responsive.
5. Stop/cancel and active-turn state remain accurate during a controlled long-running task.
6. `%USERPROFILE%\.codex` backup and rollback procedures are confirmed.
